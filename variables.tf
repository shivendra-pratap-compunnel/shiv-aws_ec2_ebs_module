...
variable "vpc_security_group_ids" {
  ...
  type        = list(string)
  default     = [""] #add the security group
}
...
################################################################################
variable "ebs_count" {
  description = "Determines count of the ebs volume for instance"
  type        = number
  default     = 1

  validation {
    condition     = var.ebs_count >= 1 && var.ebs_count <= 3
    error_message = "Invalid input value for ebs_count, only allowed values are min=1, max 3"
  }
}
...
###############################################
variable "ebs_size" {
  description = "Ebs size for the instance"
  type        = number
  default     = 8
}

variable "final_snapshot" {
  description = "snapshot will be created before volume deletion"
  type        = bool
  default     = false
}
...
###############################################
# EBS Attachment Variables
###############################################
variable "device_name" {
  description = "The device name to expose to the instance"
  type        = list(string)
  default     = ["/dev/sdd", "/dev/sde", "/dev/sdf", "/dev/sdg", "/dev/sdh", "/dev/sdi", "/dev/sdj", "/dev/sdk", "/dev/sdl", "/dev/sdm", "/dev/sdn", "/dev/sdo", "/dev/sdp", "/dev/sdq", "/dev/sdr", "/dev/sds", "/dev/sdt", "/dev/sdu", "/dev/sdv", "/dev/sdw", "/dev/sdx", "/dev/sdy", "/dev/sdz"]
}

variable "force_detach" {
  description = "Force the volume to detach"
  type        = bool
  default     = false
}
